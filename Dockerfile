...
# Uses the official prebuilt OpenClaw image to avoid compiling OpenClaw
# inside Cloudflare's build environment.
...
# Install Cloud Claw runtime dependencies + TigrisFS
RUN set -eux; \
    apt-get update; \
    apt-get install -y --no-install-recommends \
        ...
        curl; \
    curl -fsSL \
        "https://github.com/tigrisdata/tigrisfs/releases/download/v${TIGRISFS_VERSION}/tigrisfs_${TIGRISFS_VERSION}_linux_amd64.deb" \
        -o /tmp/tigrisfs.deb; \
...
EOFCONFIG
    ...
    echo "[INFO] Default OpenClaw config created"
fi
